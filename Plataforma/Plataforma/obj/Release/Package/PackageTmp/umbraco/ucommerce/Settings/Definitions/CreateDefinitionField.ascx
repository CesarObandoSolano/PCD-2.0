<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="CreateDefinitionField.ascx.cs" Inherits="UCommerce.Web.UI.Settings.Definitions.CreateDefinitionField" %>
<table>
	<tr>
		...
		<td>
			...
			<asp:RequiredFieldValidator meta:ResourceKey="RequiredNameValidator" ID="RequiredFieldValidator1" runat="server" ControlToValidate="NameTextBox" CssClass="validationMessage" ErrorMessage="Please enter a name.(*)" Width="100%" Display="Dynamic"/>
			<asp:RegularExpressionValidator meta:ResourceKey="NameValidator" ID="NameValidator" ControlToValidate="NameTextBox" ValidationExpression="^(?!(xml|[_\d\W]))[^ \s\W]+$" CssClass="validationMessage" ErrorMessage="Name can contain only letters and numbers(*)." Display="Dynamic" runat="server" />
